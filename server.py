...
# Dedicated logger for SQL queries generated by the LLM. Using a named
# logger (rather than print) keeps these entries tagged and easy to grep
# in Render's log stream, e.g. search "sql_audit" in the Logs tab.
...
def execute_sql_query(query: str) -> str:
    ...
    # Log every query the LLM sends, before we even validate it, so blocked
    # attempts show up too (useful for seeing exactly what the model tried).
    sql_logger.info("LLM query received: %s", clean_query)

    if not clean_query.upper().startswith("SELECT"):
        sql_logger.warning("Blocked non-SELECT query: %s", clean_query)
    ...
    try:
        ...
        sql_logger.info("Query succeeded, %d row(s) returned", len(rows))
        return json.dumps(rows, default=str)
    except Exception as e:
        sql_logger.error("Query failed: %s", str(e))
